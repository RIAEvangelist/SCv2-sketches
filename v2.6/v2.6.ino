...
void setChargingAmps(){
  ...
  if (volts > OUTPUT_VOLTS_END || volts > OUTPUT_VOLTS_MAX) {
  ...
  //stop the charger for this cycle if the min voltage is seen
...
void loop() {
  ...
  while (CAN_MSGAVAIL == Charger.checkReceive()) {
    ...
    char str[20];
    short chargerVolts=0;
    Charger.readMsgBuf(&ReceivedChargerMessageLength, ReceivedChargerMessage);
    chargerVolts=word(ReceivedChargerMessage[VoltageHighByte], ReceivedChargerMessage[VoltageLowByte]);

    //use charger volts if battery and chargers are beyond chargers tolerance of 1%
    if(chargerVolts>volts+11 || chargerVolts<volts-11){
      volts=chargerVolts;
    }
  }
