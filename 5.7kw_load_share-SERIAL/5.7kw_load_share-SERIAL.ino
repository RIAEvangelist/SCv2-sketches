...
#define can_rx  4           // rx of serial can module connect to D4
...
const unsigned long OUTPUT_WATTS_CUTBACK[]={990000,
  750000,
  660000,
  550000,
  450000,
...
void setup() {
  ...
  can.begin(can_tx, can_rx, CAN_SERIAL_BAUD);
}
...
void loop() {
  ...
  if(can.recv(&id, dta)){
      ...
      volts = volts + word(dta[VoltageHighByte], dta[VoltageLowByte]);

      //this could cause wattage to read high
      //if some chargers were on 120vac and others on 240vac
      amps = amps + word(dta[CurrentHighByte], dta[CurrentLowByte]);
  ...
  //can.send(HeartbeatMessageID, 1, 0, HeartbeatMessageLength, HeartbeatMessage);
  delay(1000);
}
